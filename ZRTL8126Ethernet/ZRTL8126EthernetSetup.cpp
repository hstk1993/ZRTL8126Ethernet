...
bool ZRTL8126::initEventSources(IOService *provider)
{
    ...
    if (msiIndex != -1) {
        DebugLog("ZRTL8126: MSI interrupt index: %d\n", msiIndex);
        
        interruptSource = IOInterruptEventSource::interruptEventSource(this, OSMemberFunctionCast(IOInterruptEventSource::Action, this, &ZRTL8126::interruptHandler), provider, msiIndex);
    }
    if (!interruptSource) {
