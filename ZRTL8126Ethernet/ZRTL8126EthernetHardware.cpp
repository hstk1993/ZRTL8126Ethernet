...
bool ZRTL8126::initPCIConfigSpace(IOPCIDevice *provider) {
  ...
  if (provider->extendedFindPCICapability(kIOPCIPCIExpressCapability,
    ...
    DebugLog(
        "ZRTL8126: PCIe link capabilities: 0x%08x, link control: 0x%04x.\n",
...
IOReturn ZRTL8126::setPowerStateWakeAction(OSObject *owner, void *arg1,
...
IOReturn ZRTL8126::setPowerStateSleepAction(OSObject *owner, void *arg1,
                                            void *arg2, void *arg3,
                                            void *arg4) {
  ZRTL8126 *ethCtlr = OSDynamicCast(ZRTL8126, owner);
  IOPCIDevice *dev;
...
bool ZRTL8126::initRTL8126() {
  ...
  for (i = 0; i < MAC_ADDR_LEN; i++) {
    ...
    origMacAddr.bytes[i] =
        currMacAddr.bytes[i]; /* keep the original MAC address */
  }
  IOLog("ZRTL8126: %s: (Chipset %d), %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
        rtl_chip_info[tp->chipset].name, tp->chipset, origMacAddr.bytes[0],
        origMacAddr.bytes[1], origMacAddr.bytes[2], origMacAddr.bytes[3],
...
/* Reset the NIC in case a tx deadlock or a pci error occurred. timerSource and
 * txQueue are stopped immediately but will be restarted by checkLinkStatus()
 * when the link has been reestablished.
 */
...
void ZRTL8126::setupRTL8126() {
  ...
  WriteReg32(RxConfig, ReadReg32(RxConfig) &
...
void ZRTL8126::setPhyMedium() {
  ...
  if (speed != SPEED_2500 && (speed != SPEED_1000) && (speed != SPEED_100) &&
      (speed != SPEED_10) && (speed != SPEED_5000)) {
    duplex = DUPLEX_FULL;
...
void ZRTL8126::disablePCIOffset99() {
  ...
  case CFG_METHOD_2:
  case CFG_METHOD_3:
    rtl8126_mac_ocp_write(tp, 0xE032,
                          rtl8126_mac_ocp_read(tp, 0xE032) & ~(BIT_0 | BIT_1));
    break;
  }
...
void ZRTL8126::powerDownPLL() {
  ...
  if (tp->wol_enabled == WOL_ENABLED || tp->EnableKCPOffload) {
    ...
    WriteReg32(RxConfig, ReadReg32(RxConfig) | AcceptBroadcast |
...
void ZRTL8126::configPhyHardware8126a2() {
  ...
  rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0XAC3C, BIT_15 | BIT_14 | BIT_12,
                                        BIT_13);
  rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xAC42, BIT_9,
                                        BIT_8 | BIT_7 | BIT_6);
...
void ZRTL8126::configPhyHardware8126a3() {
  ...
  rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xADE8,
