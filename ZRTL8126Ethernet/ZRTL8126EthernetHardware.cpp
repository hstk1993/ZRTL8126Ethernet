...
bool ZRTL8126::initPCIConfigSpace(IOPCIDevice *provider)
{
        ...
        {
                IOLog("ZRTL8126: PCI power management unsupported.\n");
        }
        ...
        if (provider->extendedFindPCICapability(kIOPCIPCIExpressCapability, &pcieCapOffset))
        {
                ...
                DebugLog("ZRTL8126: PCIe link capabilities: 0x%08x, link control: 0x%04x.\n", pcieLinkCap, pcieLinkCtl);

                if (linuxData.configASPM == 0)
                {
                        IOLog("ZRTL8126: Disable PCIe ASPM.\n");
                        provider->setASPMState(this, 0);
                }
                else
                {
                        IOLog("ZRTL8126: Warning: Enable PCIe ASPM.\n");
        ...
        if (!baseMap)
        {
                IOLog("ZRTL8126: region #2 not an MMIO resource, aborting.\n");
                goto done;
        }
...
IOReturn ZRTL8126::setPowerStateWakeAction(OSObject *owner, void *arg1, void *arg2, void *arg3, void *arg4)
{
...
IOReturn ZRTL8126::setPowerStateSleepAction(OSObject *owner, void *arg1, void *arg2, void *arg3, void *arg4)
{
        ZRTL8126 *ethCtlr = OSDynamicCast(ZRTL8126, owner);
        IOPCIDevice *dev;
        UInt16 val16;
...
IOReturn ZRTL8126::identifyChip()
{
...
bool ZRTL8126::initRTL8126()
{
        ...
        if (identifyChip())
        {
                IOLog("ZRTL8126: Unsupported chip found. Aborting...\n");
                goto done;
        }
        ...
        for (i = 0; i < tp->HwSuppIndirTblEntries; i++)
                tp->rss_indir_tbl[i] = (uint8_t)(i % tp->HwSuppNumRxQueues);
        static const uint8_t default_key[RTL8126_RSS_KEY_SIZE] = {
        ...
        memcpy(tp->rss_key, default_key, sizeof(default_key));
        ...
        {
                IOLog("ZRTL8126: Get macAddr %s \n", macAddr);
                rtl8126_rar_set(tp, macAddr);
        }
        else
        {
                IOLog("ZRTL8126: Using fallback MAC.\n");
        ...
        IOLog("ZRTL8126: %s: (Chipset %d), %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
        ...
        if (wolCapable)
                IOLog("ZRTL8126: Device is WoL capable.\n");
...
void ZRTL8126::setupRTL8126()
{
            ...
            tp->mcfg == CFG_METHOD_3)
        {
                ...
                /* enable RSS. */
                rss_ctrl = (tp->HwSuppNumRxQueues >= 32) ? 5 : 
                                (tp->HwSuppNumRxQueues >= 16) ? 4 : 
                                (tp->HwSuppNumRxQueues >= 8)  ? 3 : 2;
                rss_ctrl &= (BIT_0 | BIT_1 | BIT_2);
                rss_ctrl <<= RSS_CPU_NUM_OFFSET;

                /* Perform hash on these packet types 待确定*/
                rss_ctrl |= RSS_CTRL_TCP_IPV4_SUPP | RSS_CTRL_IPV4_SUPP | RSS_CTRL_IPV6_SUPP | RSS_CTRL_IPV6_EXT_SUPP | RSS_CTRL_TCP_IPV6_SUPP | RSS_CTRL_TCP_IPV6_EXT_SUPP | RSS_CTRL_UDP_IPV4_SUPP | RSS_CTRL_UDP_IPV6_SUPP |
                            RSS_CTRL_UDP_IPV6_EXT_SUPP;
                UInt32 hash_mask_len = 7;
                rss_ctrl |= (hash_mask_len &= (BIT_0 | BIT_1 | BIT_2)) << RSS_MASK_BITS_OFFSET;
                WriteReg32(RSS_CTRL_8125, rss_ctrl);

                UInt32 i;
                UInt32 reta = 0;
                UInt32 indir_tbl_reg = RSS_INDIRECTION_TBL_8125_V2;
                UInt8 *indir_tbl = tp->rss_indir_tbl;
                for (i = 0; i < tp->HwSuppIndirTblEntries; i++) {
                                reta |= indir_tbl[i] << (i & 0x3) * 8;
                                if ((i & 3) == 3) {
                                        WriteReg32(indir_tbl_reg, reta);
                                        indir_tbl_reg += 4;
                                        reta = 0;
                                }
                        }

                const UInt16 rss_key_reg = RSS_KEY_8125;
                UInt32 l, rss_key_size = sizeof(tp->rss_key);
                UInt32 *rss_key = (UInt32*)tp->rss_key;

                /* Write redirection table to HW */
                for (l = 0; l < rss_key_size; l+=4)
                        WriteReg32(rss_key_reg + l, rss_key[l / 4]);

                UInt16 q_ctrl;
                UInt16 rx_q_num;
                rx_q_num = (tp->HwSuppNumRxQueues >= 32) ? 5 : 
                                (tp->HwSuppNumRxQueues >= 16) ? 4 : 
                                (tp->HwSuppNumRxQueues >= 8)  ? 3 : 2;;
                rx_q_num &= (BIT_0 | BIT_1 | BIT_2);
                rx_q_num <<= 2;
                q_ctrl = ReadReg16(Q_NUM_CTRL_8125);
                q_ctrl &= ~(BIT_2 | BIT_3 | BIT_4);
                q_ctrl |= rx_q_num;
                WriteReg16(Q_NUM_CTRL_8125, q_ctrl);
        ...
        DebugLog("ZRTL8126: setup() <===\n");

        udelay(10);
}

void ZRTL8126::setPhyMedium()
{
        ...
        if ((linuxData.eee_adv_t != 0) && (autoneg == AUTONEG_ENABLE))
        {
                ...
                DebugLog("ZRTL8126: Enable EEE support.\n");
        }
        else
        {
                rtl8126_disable_eee(tp);
                DebugLog("ZRTL8126: Disable EEE support.\n");
        }
        // Disable Giga Lite
...
void ZRTL8126::setOffset79(UInt8 setting)
{
        UInt8 deviceControl;
...
void ZRTL8126::csiFun0WriteByte(UInt32 addr, UInt8 value)
{
        struct rtl8126_private *tp = &linuxData;
...
void ZRTL8126::enablePCIOffset99()
{
        struct rtl8126_private *tp = &linuxData;
        u32 csi_tmp;
...
void ZRTL8126::disablePCIOffset99()
{
        struct rtl8126_private *tp = &linuxData;
...
void ZRTL8126::setPCI99_180ExitDriverPara()
{
        struct rtl8126_private *tp = &linuxData;
...
void ZRTL8126::setPCI99_ExitDriverPara()
{
        struct rtl8126_private *tp = &linuxData;
...
UInt16 ZRTL8126::getEEEMode()
{
        ...
        if (eeeCap)
        {
                ...
                DebugLog("ZRTL8126: EEE supported: %u\n", sup);

                /* Get advertisement EEE. */
                adv = rtl8126_mdio_direct_read_phy_ocp(tp, 0xA5D0);
                DebugLog("ZRTL8126: EEE advertised: %u\n", adv);

                /* Get LP advertisement EEE. */
                lpa = rtl8126_mdio_direct_read_phy_ocp(tp, 0xA5D2);
                DebugLog("ZRTL8126: EEE link partner: %u\n", lpa);

                ena = rtl8126_mac_ocp_read(tp, 0xE040);
                ena &= BIT_1 | BIT_0;
                DebugLog("ZRTL8126: EEE enabled: %u\n", ena);

                eee = (sup & adv & lpa);
        }
        return eee;
}
void ZRTL8126::exitOOB()
{
        struct rtl8126_private *tp = &linuxData;
        UInt16 data16;
...
void ZRTL8126::configPhyHardware8126a1()
{
        struct rtl8126_private *tp = &linuxData;
...
void ZRTL8126::configPhyHardware8126a2()
{
        struct rtl8126_private *tp = &linuxData;
...
void ZRTL8126::configPhyHardware8126a3()
{
        struct rtl8126_private *tp = &linuxData;
